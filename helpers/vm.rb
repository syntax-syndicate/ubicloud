...
class Clover
  ...
  def generate_vm_options
    ...
    options.add_option(name: "location", values: Option.locations(feature_flags: @project.feature_flags).map(&:id))
    ...
    options.add_option(name: "private_subnet_id", values: subnets, parent: "location") do |location_id, private_subnet|
      private_subnet[:location_id] == location_id
    end
