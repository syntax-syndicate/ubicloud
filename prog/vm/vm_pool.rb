...
class Prog::Vm::VmPool < Prog::Base
  ...
  label def wait
    if vm_pool.size - vm_pool.vms.count > 0
      idle_cpus = VmHost.where(allocation_state: "accepting", arch: vm_pool.arch, location_id: Location.where(name: ["github-runners", "hetzner-hel1", "hetzner-fsn1"]).select_map(:id)).select_map { sum((total_cores - used_cores) * total_cpus / total_cores) }.first.to_i
