...
class Prog::Vm::HostNexus < Prog::Base
  ...
  def self.assemble(sshable_hostname, location_id: "caa7a807-36c5-8420-a75c-f906839dad71", net6: nil, ndp_needed: false, provider_name: nil, server_identifier: nil, spdk_version: Config.spdk_version, default_boot_images: [])
    DB.transaction do
      unless Location[location_id]
        raise "Location #{location_id} not found"
      end

      ubid = VmHost.generate_ubid

      Sshable.create(host: sshable_hostname) { _1.id = ubid.to_uuid }
      vmh = VmHost.create(location_id: location_id, net6: net6, ndp_needed: ndp_needed) { _1.id = ubid.to_uuid }
