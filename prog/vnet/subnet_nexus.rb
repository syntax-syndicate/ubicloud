...
class Prog::Vnet::SubnetNexus < Prog::Base
  ...
  def self.assemble(project_id, name: nil, location_id: "caa7a807-36c5-8420-a75c-f906839dad71", ipv6_range: nil, ipv4_range: nil, allow_only_ssh: false, firewall_id: nil)
  ...
  def self.random_private_ipv6(location, project)
    ...
    selected_addr = random_private_ipv6(location, project) if project.private_subnets_dataset[Sequel[:net6] => selected_addr.to_s, :location_id => location.id]
  ...
  def self.random_private_ipv4(location, project, cidr_size = 26)
    ...
    selected_addr = random_private_ipv4(location, project, cidr_size) if PrivateSubnet::BANNED_IPV4_SUBNETS.any? { _1.rel(selected_addr) } || project.private_subnets_dataset[Sequel[:net4] => selected_addr.to_s, :location_id => location.id]
