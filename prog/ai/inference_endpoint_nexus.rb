...
class Prog::Ai::InferenceEndpointNexus < Prog::Base
  ...
  def self.assemble_with_model(project_id:, location_id:, name:, model_id:,
    ...
    fail "Location with id #{location_id} not found" unless Location[location_id]
  ...
  def self.assemble(project_id:, location_id:, boot_image:, name:, vm_size:, storage_volumes:, model_name:,
    ...
    DB.transaction do
      ...
      firewall = internal_project.firewalls_dataset.where(location_id: location_id).where(Sequel[:firewall][:name] => "inference-endpoint-firewall").first
      ...
      subnet_s = Prog::Vnet::SubnetNexus.assemble(internal_project.id, name: ubid.to_s, location_id: location_id, firewall_id: firewall.id)
