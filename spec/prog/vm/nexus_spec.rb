  ...
  describe "#destroy" do
    ...
    context "when has vm_host" do
      ...
      it "absorbs an already deleted errors as a success" do
        expect(sshable).to receive(:cmd).with("systemctl stop #{nx.vm_name}", timeout: 10).and_raise(
      ...
      it "raises other stop errors" do
        ...
        expect(sshable).to receive(:cmd).with("systemctl stop #{nx.vm_name}", timeout: 10).and_raise(ex)
      ...
      it "raises other stop-dnsmasq errors" do
        ex = Sshable::SshError.new("stop", "", "unknown error", 1, nil)
        expect(sshable).to receive(:cmd).with("systemctl stop #{nx.vm_name}", timeout: 10)
