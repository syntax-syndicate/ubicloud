...
RSpec.describe Prog::Vm::Nexus do
  ...
  describe ".assemble" do
    let(:ps) {
      PrivateSubnet.create(name: "ps", location_id: "caa7a807-36c5-8420-a75c-f906839dad71", net6: "fd10:9b0b:6b4b:8fbb::/64",
    ...
    it "adds the VM to a private subnet if nic_id is passed" do
      ...
      described_class.assemble("some_ssh_key", prj.id, nic_id: nic.id, location_id: "caa7a807-36c5-8420-a75c-f906839dad71")
    end
  ...
  describe "#prep" do
    ...
    it "generates and passes a params json if prep command is not started yet" do
      ...
      ps = instance_double(PrivateSubnet, location_id: "caa7a807-36c5-8420-a75c-f906839dad71", net4: NetAddr::IPv4Net.parse("10.0.0.0/26"))
  ...
  describe "#start" do
    ...
    it "allocates with expected parameters" do
      expect(Scheduling::Allocator).to receive(:allocate).with(
        ...
        location_filter: ["caa7a807-36c5-8420-a75c-f906839dad71"],
        location_preference: [],
        gpu_count: 0
      )
      expect { nx.start }.to hop("create_unix_user")
    ...
    it "considers EU locations for github-runners" do
      ...
      expect(Scheduling::Allocator).to receive(:allocate).with(
        ...
        host_filter: [],
        host_exclusion_filter: [],
        location_filter: ["6b9ef786-b842-8420-8c65-c25e3d4bdf3d", "caa7a807-36c5-8420-a75c-f906839dad71", "1f214853-0bc4-8020-b910-dffb867ef44f"],
