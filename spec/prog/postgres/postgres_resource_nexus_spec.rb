...
RSpec.describe Prog::Postgres::PostgresResourceNexus do
  ...
  describe ".assemble" do
    ...
    it "validates input" do
      ...
      expect {
        described_class.assemble(project_id: "26820e05-562a-4e25-a51b-de5f78bd00af", location_id: "caa7a807-36c5-8420-a75c-f906839dad71", name: "pg-name", target_vm_size: "standard-2", target_storage_size_gib: 128)
      }.to raise_error RuntimeError, "No existing project"

      expect {
        described_class.assemble(project_id: customer_project.id, location_id: "caa7a807-36c5-8420-a75c-f906839dad71", name: "pg/server/name", target_vm_size: "standard-2", target_storage_size_gib: 128)
      }.to raise_error Validation::ValidationFailed, "Validation failed for following fields: name"

      expect {
        described_class.assemble(project_id: customer_project.id, location_id: "caa7a807-36c5-8420-a75c-f906839dad71", name: "pg-name", target_vm_size: "standard-128", target_storage_size_gib: 128)
      }.to raise_error Validation::ValidationFailed, "Validation failed for following fields: size"

      expect {
        described_class.assemble(project_id: customer_project.id, location_id: "caa7a807-36c5-8420-a75c-f906839dad71", name: "pg-name", target_vm_size: "standard-2", target_storage_size_gib: 128)
      }.not_to raise_error

      expect {
        described_class.assemble(project_id: customer_project.id, location_id: "caa7a807-36c5-8420-a75c-f906839dad71", name: "pg-name", target_vm_size: "standard-2", target_storage_size_gib: 128, parent_id: "69c0f4cd-99c1-8ed0-acfe-7b013ce2fa0b")
      }.to raise_error RuntimeError, "No existing parent"

      expect {
        parent = described_class.assemble(project_id: customer_project.id, location_id: "caa7a807-36c5-8420-a75c-f906839dad71", name: "pg-parent-name", target_vm_size: "standard-2", target_storage_size_gib: 128).subject
        expect(PostgresResource).to receive(:[]).with(parent.id).and_return(parent)
        described_class.assemble(project_id: customer_project.id, location_id: "caa7a807-36c5-8420-a75c-f906839dad71", name: "pg-name", target_vm_size: "standard-2", target_storage_size_gib: 128, parent_id: parent.id, restore_target: Time.now)
    ...
    it "does not allow giving different version than parent for restore" do
      expect(Config).to receive(:postgres_service_project_id).and_return(postgres_project.id).at_least(:once)
      parent = described_class.assemble(project_id: customer_project.id, location_id: "caa7a807-36c5-8420-a75c-f906839dad71", name: "pg-parent-name", target_vm_size: "standard-2", target_storage_size_gib: 128, version: "16").subject
      expect(PostgresResource).to receive(:[]).with(parent.id).and_return(parent)
      expect {
        described_class.assemble(project_id: customer_project.id, location_id: "caa7a807-36c5-8420-a75c-f906839dad71", name: "pg-name", target_vm_size: "standard-2", target_storage_size_gib: 128, parent_id: parent.id, version: "17", restore_target: Time.now)
    ...
    it "validates storage size during restore if the storage size is different from the parent" do
      ...
      expect {
        described_class.assemble(project_id: customer_project.id, location_id: "caa7a807-36c5-8420-a75c-f906839dad71", name: "pg-name", target_vm_size: "standard-2", target_storage_size_gib: 1024, parent_id: parent.id, restore_target: Time.now)
      }.not_to raise_error

      expect {
        described_class.assemble(project_id: customer_project.id, location_id: "caa7a807-36c5-8420-a75c-f906839dad71", name: "pg-name", target_vm_size: "standard-2", target_storage_size_gib: 2048, parent_id: parent.id, restore_target: Time.now)
    ...
    it "passes timeline of parent resource if parent is passed" do
      ...
      parent = described_class.assemble(project_id: customer_project.id, location_id: "caa7a807-36c5-8420-a75c-f906839dad71", name: "pg-name", target_vm_size: "standard-2", target_storage_size_gib: 128).subject
      restore_target = Time.now
      expect(parent.timeline).to receive(:earliest_restore_time).and_return(restore_target - 10 * 60)
      expect(PostgresResource).to receive(:[]).with(parent.id).and_return(parent)
      expect(Prog::Postgres::PostgresServerNexus).to receive(:assemble).with(hash_including(timeline_id: parent.timeline.id, timeline_access: "fetch")).and_return(instance_double(Strand, subject: postgres_resource.representative_server))

      described_class.assemble(project_id: customer_project.id, location_id: "caa7a807-36c5-8420-a75c-f906839dad71", name: "pg-name-2", target_vm_size: "standard-2", target_storage_size_gib: 128, parent_id: parent.id, restore_target: restore_target)
    end
  end
