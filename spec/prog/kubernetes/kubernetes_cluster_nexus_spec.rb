...
RSpec.describe Prog::Kubernetes::KubernetesClusterNexus do
  ...
  let(:subnet) { PrivateSubnet.create(net6: "0::0", net4: "127.0.0.1", name: "x", location_id: "caa7a807-36c5-8420-a75c-f906839dad71", project_id: project.id) }
  ...
  describe ".assemble" do
    it "validates input" do
      expect {
        described_class.assemble(project_id: SecureRandom.uuid, name: "k8stest", version: "v1.32", location_id: "caa7a807-36c5-8420-a75c-f906839dad71", cp_node_count: 3, private_subnet_id: subnet.id)
      }.to raise_error RuntimeError, "No existing project"

      expect {
        described_class.assemble(version: "v1.30", project_id: project.id, name: "k8stest", location_id: "caa7a807-36c5-8420-a75c-f906839dad71", cp_node_count: 3, private_subnet_id: subnet.id)
      }.to raise_error RuntimeError, "Invalid Kubernetes Version"

      expect {
        described_class.assemble(name: "Uppercase", version: "v1.32", project_id: project.id, location_id: "caa7a807-36c5-8420-a75c-f906839dad71", cp_node_count: 3, private_subnet_id: subnet.id)
      }.to raise_error Validation::ValidationFailed, "Validation failed for following fields: name"

      expect {
        described_class.assemble(name: "hyph_en", version: "v1.32", project_id: project.id, location_id: "caa7a807-36c5-8420-a75c-f906839dad71", cp_node_count: 3, private_subnet_id: subnet.id)
      }.to raise_error Validation::ValidationFailed, "Validation failed for following fields: name"

      expect {
        described_class.assemble(name: "onetoolongnameforatestkubernetesclustername", version: "v1.32", project_id: project.id, location_id: "caa7a807-36c5-8420-a75c-f906839dad71", cp_node_count: 3, private_subnet_id: subnet.id)
      }.to raise_error Validation::ValidationFailed, "Validation failed for following fields: name"

      p = Project.create(name: "another")
      subnet.update(project_id: p.id)
      expect {
        described_class.assemble(name: "normalname", project_id: project.id, location_id: "caa7a807-36c5-8420-a75c-f906839dad71", cp_node_count: 3, private_subnet_id: subnet.id)
      }.to raise_error RuntimeError, "Given subnet is not available in the given project"
    end

    it "creates a kubernetes cluster" do
      st = described_class.assemble(name: "k8stest", version: "v1.31", private_subnet_id: subnet.id, project_id: project.id, location_id: "caa7a807-36c5-8420-a75c-f906839dad71", cp_node_count: 3, target_node_size: "standard-8", target_node_storage_size_gib: 100)

      kc = st.subject
      expect(kc.name).to eq "k8stest"
      expect(kc.ubid).to start_with("kc")
      expect(kc.version).to eq "v1.31"
      expect(kc.location_id).to eq "caa7a807-36c5-8420-a75c-f906839dad71"
    ...
    it "has defaults for node size, storage size, version and subnet" do
      st = described_class.assemble(name: "k8stest", project_id: project.id, location_id: "caa7a807-36c5-8420-a75c-f906839dad71", cp_node_count: 3)
      kc = st.subject
