...
RSpec.describe Authorization do
  ...
  let(:vms) {
    (0..3).map do |index|
      ps = Prog::Vnet::SubnetNexus.assemble(projects[index / 2].id, name: "vm#{index}-ps", location_id: "caa7a807-36c5-8420-a75c-f906839dad71").subject
      Prog::Vm::Nexus.assemble("key", projects[index / 2].id, name: "vm#{index}", private_subnet_id: ps.id)
    end.map(&:subject)
  }
  let(:pg) {
    Prog::Postgres::PostgresResourceNexus.assemble(
      project_id: projects[0].id, location_id: "caa7a807-36c5-8420-a75c-f906839dad71", name: "pg0", target_vm_size: "standard-2", target_storage_size_gib: 128
    ).subject
  }
