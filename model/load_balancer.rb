...
class LoadBalancer < Sequel::Model
  ...
  many_to_many :vms_to_dns, class: :Vm, left_key: :load_balancer_id, right_key: :vm_id, join_table: :load_balancer_vm_port, conditions: Sequel.~(state: Sequel.any_type(["evacuating", "detaching"], :lb_node_state)), distinct: true
  one_to_many :active_vm_ports, class: :LoadBalancerVmPort, key: :load_balancer_id, conditions: {state: "up"}
  ...
  one_to_many :vm_ports, key: :load_balancer_id, class: :LoadBalancerVmPort
  ...
  def add_vm(vm)
    DB.transaction do
      ...
      ports.each { |port| LoadBalancerVmPort.create_with_id(vm_id: vm.id, load_balancer_id: id, load_balancer_port_id: port.id) }
  ...
  def detach_vm(vm)
    vm_ports_dataset.where(vm_id: vm.id, state: Sequel.any_type(["up", "down", "evacuating"], :lb_node_state)).update(state: "detaching")
  ...
  def evacuate_vm(vm)
    DB.transaction do
      vm_ports_dataset.where(vm_id: vm.id, state: Sequel.any_type(["up", "down"], :lb_node_state)).update(state: "evacuating")
      Strand.create_with_id(prog: "Vnet::CertServer", label: "remove_cert_server", stack: [{subject_id: id, vm_id: vm.id}], parent_id: id)
      incr_update_load_balancer
      incr_rewrite_dns_records
    end
  ...
  def remove_vm(vm)
    vm_ports_dataset.where(vm_id: vm.id).destroy
...
#  health_check_up_threshold   | integer        | DEFAULT 5
#  health_check_down_threshold | integer        | DEFAULT 3
#  health_check_protocol       | lb_hc_protocol | DEFAULT 'http'::lb_hc_protocol
# Indexes:
...
#  load_balancer_vm_port | load_balancer_vm_port_load_balancer_id_fkey | (load_balancer_id) REFERENCES load_balancer(id) ON DELETE CASCADE
